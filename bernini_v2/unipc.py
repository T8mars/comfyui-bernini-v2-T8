...
def _coefficients(h: torch.Tensor, order: int, device: torch.device) -> tuple[torch.Tensor, torch.Tensor]:
...
def _uni_p_bh2_update(
    ...
    h = _lambda(sigmas[step_index + 1].to(sample.device)) - _lambda(sigmas[step_index].to(sample.device))
    ...
    for history_index in range(1, order):
        ...
        rk = (_lambda(sigmas[previous_step].to(sample.device)) - _lambda(sigmas[step_index].to(sample.device))) / h
        rks.append(rk)
...
def _uni_c_bh2_update(
    ...
    sigma_t_value = sigmas[step_index].to(device=this_sample.device, dtype=torch.float32)
    sigma_s0_value = sigmas[step_index - 1].to(device=this_sample.device, dtype=torch.float32)
    ...
    for history_index in range(1, order):
        ...
        rk = (_lambda(sigmas[previous_step].to(this_sample.device)) - _lambda(sigma_s0_value)) / h
        rks.append(rk)
        d1s.append((previous - m0) / rk)
...
def sample_flow_unipc_bh2(
    ...
    for step_index in range(total_steps):
        ...
        last_sample = sample
        sample = _uni_p_bh2_update(sample, model_outputs, sigmas, step_index, this_order)
