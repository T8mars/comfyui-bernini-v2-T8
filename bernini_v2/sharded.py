...
def load_checkpoint_state_dict(path: str | Path) -> dict[str, torch.Tensor]:
    ...
    raise ValueError(f"expected .safetensors or .safetensors.index.json, got {checkpoint}")
...
def component_checkpoint(root: str | Path, component: str) -> Path:
    ...
    raise ValueError(f"{component_dir} contains multiple single-file checkpoints: {[path.name for path in candidates]}")
