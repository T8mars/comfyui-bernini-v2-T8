...
def build_custom_attention_mask(token_type: torch.Tensor, token_segment_ids: torch.Tensor) -> torch.Tensor:
    ...
    causal = torch.tril(torch.ones(length, length, device=token_type.device, dtype=torch.bool)).unsqueeze(0)
...
class BerniniTemplate:
    ...
    def __init__(self, tokenizer, max_visual_items: int = 64):
        ...
        self.visual_input_pads = [f"<|visual_input_token_pad_{index}|>" for index in range(max_visual_items)]
        self.visual_output_pads = [f"<|visual_output_token_pad_{index}|>" for index in range(max_visual_items)]
        tokenizer.add_special_tokens({"additional_special_tokens": self.visual_input_pads + self.visual_output_pads})
        self.visual_input_ids = tokenizer.convert_tokens_to_ids(self.visual_input_pads)
        self.visual_output_ids = tokenizer.convert_tokens_to_ids(self.visual_output_pads)
    ...
    def encode(
        ...
        *,
        ...
        messages = [{"role": "system", "content": SYSTEM_PROMPTS.get(task, SYSTEM_PROMPTS["default"])}]
        content = ""
        text_content = ""
        previous_has_loss = False
        visual_id_to_type: dict[int, int] = {}
        visual_indicator_maps: dict[int, int] = {}
        ...
        def flush(has_loss: bool) -> None:
            nonlocal content
            if content.strip():
                messages.append({"role": "assistant" if has_loss else "user", "content": content})
            content = ""
        ...
        for raw_message in conversations:
            ...
            if message_type == "special_token":
                continue
            has_loss = bool(message.get("has_loss", message_type in ("image_gen", "video_gen", "frame_gen")))
            ...
            if message_type in ("image", "image_gen"):
                ...
                if include:
                    content += self._visual_pattern(token_count, visual_id, output=has_loss)
                    visual_types.append(0)
                    visual_source_indices.append(image_id)
                    pad_ids = self.visual_output_ids if has_loss else self.visual_input_ids
                    if has_loss:
                        indicator_id += 1
                    visual_indicator_maps[pad_ids[visual_id]] = indicator_id
        ...
        flex_token_types = -torch.ones_like(ids, dtype=torch.int)
        ...
        for current_id, token_id in enumerate(self.visual_input_ids):
            ...
            ids[selected] = self.image_pad_id if visual_id_to_type[current_id] == 0 else self.video_pad_id
        ...
        for current_id, token_id in enumerate(self.visual_output_ids):
            ...
            flex_token_types[selected] = visual_indicator_maps[token_id]
            visual_output_mask[selected] = True
            token_segment_ids[selected] = current_id + 1
            ids[selected] = self.image_pad_id if visual_id_to_type[current_id] == 0 else self.video_pad_id

        attention_4d = build_custom_attention_mask(token_types.unsqueeze(0), token_segment_ids.unsqueeze(0))
        return {
            "input_ids": ids,
            ...
            "flex_token_types": flex_token_types,
...
def build_conversation(
    ...
    *,
    ...
    messages: list[dict[str, object]] = [{"type": "special_token", "text": "[CLS]", "has_loss": 0}]
...
def token_count_dict(image_counts: Sequence[int], video_counts: Sequence[int]) -> defaultdict[str, list[int]]:
    output: defaultdict[str, list[int]] = defaultdict(list)
    output["image"].extend(image_counts)
    output["video"].extend(video_counts)
    return output
