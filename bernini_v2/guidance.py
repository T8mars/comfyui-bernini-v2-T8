...
def append_dims(value: torch.Tensor, ndim: int) -> torch.Tensor:
    ...
    if value.ndim > ndim:
        raise ValueError(f"cannot broadcast a {value.ndim}D sigma over a {ndim}D latent")
...
def apg_delta(
    ...
    *,
    ...
    norm_squared = (reference_flat * reference_flat).sum(dim=1, keepdim=True).clamp_min(eps)
    ...
    return parallel_scale * parallel.reshape_as(delta) + orthogonal_scale * orthogonal.reshape_as(delta)
...
def compose_denoised_guidance(
    ...
    **kwargs,
    ...
    velocities = {name: denoised_to_velocity(prediction, sample, sigma) for name, prediction in predictions.items()}
